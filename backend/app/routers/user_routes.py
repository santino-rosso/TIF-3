...
async def register_user(request: Request, usuario: UserCreate):
...
@limiter.limit("5/minute")
async def login_user(request: Request, usuario: OAuth2PasswordRequestForm = Depends()):
...
async def refresh_token(request: Request, body: UserLogout):
    new_token = await refresh_access_token(body.refresh_token)
    return {
